...
class AsyncESClient(httpClient: AsyncHttpClient, url: String, scriptTemplateIsAvailable: Boolean = false)
  ...
  def searchAsync(config: ESConfig)(f: SearchSourceBuilder => Unit): Future[Either[Map[String, Any], Map[String, Any]]] = {
  ...
  def findAsync[T](config: ESConfig)(f: SearchSourceBuilder => Unit)(implicit c: ClassTag[T]): Future[Option[(String, T)]] = {
  ...
  def findAsListAsync[T](config: ESConfig)(f: SearchSourceBuilder => Unit)(implicit c: ClassTag[T]): Future[List[(String, T)]] = {
    searchAsync(config)(f).map {
      case Left(x)  => throw new RuntimeException(x("error").toString)
  ...
  def findAllAsListAsync[T](config: ESConfig)(f: SearchSourceBuilder => Unit)(implicit c: ClassTag[T]): Future[List[(String, T)]] = {
  ...
  def listAsync[T](config: ESConfig)(f: SearchSourceBuilder => Unit)(implicit c: ClassTag[T]): Future[ESSearchResult[T]] = {
    searchAsync(config)(f).map {
      case Left(x)  => throw new RuntimeException(x("error").toString)
  ...
  def listAllAsync[T](config: ESConfig)(f: SearchSourceBuilder => Unit)(implicit c: ClassTag[T]): Future[ESSearchResult[T]] = {
    countAsIntAsync(config)(f).flatMap { count =>
